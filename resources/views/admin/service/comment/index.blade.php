...
@section('content')
    <section class="mb-2 d-flex justify-content-between align-items-center mt-4 mb-2">
        ...
        <form action="{{ route('admin.service.comment.index', $service) }}" method="GET" class="w-25 d-flex">
            ...
            <tbody>
                @forelse ($comments as $comment)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.service.comment.show', ['service' => $service, 'comment' => $comment]) }}"
                                class="btn btn-sm btn-info">نمایش</a>
                            <form action="{{ route('admin.service.comment.changeStatus', ['service' => $service, 'comment' => $comment]) }}" class="d-inline"
                                method="POST">
                                @csrf
                                <button class="btn btn-warning btn-sm" type="submit">تغییر وضعیت</button>
                            </form>
                            <form class="d-inline" action="{{ route('admin.service.comment.delete', ['service' => $service, 'comment' => $comment]) }}" method="POST">
                                @method('DELETE')
                                @csrf
